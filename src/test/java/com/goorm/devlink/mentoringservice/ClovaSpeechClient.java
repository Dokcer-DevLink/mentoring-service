...
public class ClovaSpeechClient {
    ...
    private static final String AWS_ACCESSKEY = "AKIAVU53PT5HYIUPCHWN";
    private static final String AWS_SECRETKEY = "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";
    private static final String bucketName = "speech-bucket-123";
...
//            final String result = clovaSpeechClient.upload(new File("/Users/kangmingu/Desktop/devlink_test_3min.mp3"), requestEntity);
        final String result = clovaSpeechClient.url("https://speech-bucket-123.s3.ap-northeast-2.amazonaws.com/devlink_test_3min.mp3", requestEntity); // 외부 버킷에 있는 파일 인식용
        ...
        for (JsonElement jsonElement : jsonArray) {
            ...
            String speaker = obj.get("speaker").getAsJsonObject().get("name").toString().replaceAll("\"","");
            String text = jsonElement.getAsJsonObject().get("text").toString().replaceAll("\"","");

            if(curSpeaker.equals(speaker)){
                curText.append("").append(text);
            }else{
                if(curSpeaker.isEmpty()){
                    curSpeaker = speaker;
                    curText.append(text);
                }
                else{
                    parsedResult.append("개발자").append(curSpeaker).append(":").append(" ").append(curText.toString()).append("\n");
                    curSpeaker = speaker;
                    curText = new StringBuilder(text);
                }
            }
        }
        parsedResult.append("개발자").append(curSpeaker).append(":").append(" ").append(curText.toString());
        System.out.println(parsedResult.toString());

    }


    public static AmazonS3Client amazonS3Client(AwsConfigVo awsConfigVo){
